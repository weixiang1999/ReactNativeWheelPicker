...
import { hourTo24Format, hourTo12Format, pickerDateArray, getHoursArray, increaseDateByDays, getFiveMinutesArray, getAmArray, } from "./Utils";
const millisecondsPerDay = 1000 * 60 * 60 * 24;
const HOUR = 60;
export default class DatePicker extends React.Component {
    constructor(props) {
        super(props);
        this.onDaySelected = (position) => {
            ...
            if (data[position] === "Today") {
                ...
                selectedDate = increaseDateByDays(new Date(), this.props.startDate ? daysAfterSelectedDate + position : position);
            }
            selectedDate.setHours(hours);
            selectedDate.setMinutes(minutes);
            this.onDateSelected(selectedDate);
        };
        this.onHourSelected = (position) => {
        ...
        this.onMinuteSelected = (position) => {
            const selectedDate = this.state.selectedDate;
        ...
        const initMinuteInex = Math.round(selectedDate.getMinutes() / (HOUR / minutesCount));
    ...
    render() {
        const { startDate, days, daysCount, hours, minutes, format24, backgroundColor, hideDate, hideHours, hideMinutes, hideAM, format, } = this.props;
        const { initHourInex, initDayInex, initMinuteInex } = this.state;
        return (<View style={[styles.container, { backgroundColor }]}>
        {!hideDate && (<WheelPicker style={styles.dateWheelPicker} {...this.props} data={days || pickerDateArray(startDate, daysCount, format)} onItemSelected={this.onDaySelected} initPosition={initDayInex}/>)}
        {!hideHours && (<WheelPicker style={styles.wheelPicker} {...this.props} isCyclic data={hours || getHoursArray(format24)} onItemSelected={this.onHourSelected} initPosition={initHourInex}/>)}
        {!hideMinutes && (<WheelPicker style={styles.wheelPicker} {...this.props} isCyclic data={minutes || getFiveMinutesArray()} onItemSelected={this.onMinuteSelected} initPosition={initMinuteInex}/>)}
        {!this.props.format24 && !hideAM && this.renderAm()}
      </View>);
    }
    renderAm() {
        const { initAmInex } = this.state;
        return (<WheelPicker style={styles.wheelPicker} {...this.props} data={getAmArray()} onItemSelected={this.onAmSelected} initPosition={initAmInex}/>);
    }
